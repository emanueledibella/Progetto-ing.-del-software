...
public class OrdersListEditOrders implements Initializable {
    ...
    public OrdersListEditOrders(UserControl userControl) {
                ...
                final TableCell<TableViewWrapper, Void> cell = new TableCell<TableViewWrapper, Void>() {
...

                    {
                        btn.setOnAction((ActionEvent event) -> {
                            ...
                            // Controlla se la data di consegna dell'ordine scelto è prossima di due giorni o meno alla data odierna
                            LocalDate now = LocalDate.now();
                            Duration diff = Duration.between(now.atStartOfDay(), orderControl.getOrder().getDataConsegna().atStartOfDay());
                            if(diff.toDays() > 2) {
                                try {
                                    FXMLLoader loader = new FXMLLoader(getClass().getResource("../javafx/OrderDetailFormEditOrders.fxml"));
                                    OrderDetailFormEditOrders orderDetailFormEditOrders = new OrderDetailFormEditOrders(userControl, orderControl);
                                    loader.setController(orderDetailFormEditOrders);
                                    Parent root;
                                    root = loader.load();
                                    Scene scene = new Scene(root);
                                    Stage stage = (Stage)((Node)event.getSource()).getScene().getWindow();
                                ...
                                try {
                                    FXMLLoader loader = new FXMLLoader(getClass().getResource("../javafx/ErrorFormEditOrders.fxml"));
                                    ErrorFormEditOrders errorFormEditOrders = new ErrorFormEditOrders(userControl);
                                    loader.setController(errorFormEditOrders);
                                    Parent root;
                                    root = loader.load();
                                    Scene scene = new Scene(root);
                                    Stage stage = (Stage)((Node)event.getSource()).getScene().getWindow();
